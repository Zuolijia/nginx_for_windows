...
static ngx_int_t ngx_http_cookie_handler(ngx_http_request_t *r)
{
	...
	if (cookies == NULL){
		...
		set_cookie->hash = 1;
		ngx_str_set(&set_cookie->key,(u_char *)"Set-Cookie");
		i++;
		//在nginx中，所有的cookie键值对都格式化为一个字符串value，形式为keyname=valuename
		ngx_sprintf(temp,"CookieName%d=Visit %d times!",i,i);
		set_cookie->value.data = temp;
		set_cookie->value.len = ngx_strlen(temp);
	}
	else {
		for(j=0;j<nelts;j++){
			ngx_sprintf(ngx_string+ngx_strlen(ngx_string),"Cookie %d : %V <br>",j+1,&cookies[j]->value);
		}
		set_cookie = ngx_list_push(&r->headers_out.headers);
		set_cookie->hash = i+1;
		ngx_str_set(&set_cookie->key,(u_char *)"Set-Cookie");
